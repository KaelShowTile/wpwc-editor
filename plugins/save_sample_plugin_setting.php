...
try {
    ...
    ");
    $deleteSettings->execute([':setting_name' => 'plugin_show_sample']);

    //Insert selected plugin
    $insertTaxStmt = $db->prepare("
        INSERT INTO `{$config['db']['prefix']}settings` 
        (setting_name, setting_value) 
        VALUES ('plugin_show_sample', :setting_value)
    ");
    
    foreach ($data['value'] as $settingvalue) {
        $insertTaxStmt->execute([':setting_value' => $settingvalue]);
    }
